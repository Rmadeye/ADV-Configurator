...
Button1=Button(window, text="Choose receptor", command=BrowsersAndCollectors.browse_receptor).grid(row=0,column=0)
Button2=Button(window, text="Choose flexible residues ", command=BrowsersAndCollectors.browse_flexres).grid(row=1,column=0)
Button3=Button(window, text="Choose ligand", command=BrowsersAndCollectors.browse_ligand).grid(row=2,column=0)
...
class ConfigMaker:
    ...
    def create_conf_file():
        outfile, logfile, ox, oy, oz, dx, dy, dz, ncpu, nmod, energy, outname, filename = BrowsersAndCollectors.collect_data()
...
def main():
    collect = Button(window, text="Collect data", command=BrowsersAndCollectors.collect_data).grid(row=14, column=1)
    exe= Button(window, text="Save cfg file", command=ConfigMaker.create_conf_file).grid(row = 15, column = 1)
    if ConfigMaker.create_conf_file == True:
